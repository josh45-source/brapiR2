...
Multi-environment trial (MET) analysis is the core workflow of a plant
breeding program: gather phenotypic observations from many locations and
years, then decompose genotype × environment (GxE) interaction to
...
With 8 environments and 4 workers, the fetch completes in roughly the
time of 2 sequential requests.
...
The study data already carries `studyDbId` and `studyName`. Join with
location and season data from the `studies` tibble for richer
environment labels.
...
mixed model with genotype × environment interaction as a random effect.
